...
### interface IOrder -  Интерфейс информация о заказе  
...
Конструктор class Card принимает имя блока, родительский контейнер и объект с колбэк функциями
...
`card:toBasket` Инициируется при клике на кнопку "В корзину" в модальном окне
...
`order:open` - Bнициируется при клике на кнопку "Оформить" в корзине Открывает окно с формой для заполнения адреса и способа оплаты
...
`orderFormErrors:change` - Инициируется при вводе данных в форму модального окна и совершает процесс валидации формы, возвращает ошибки формы
...
`modal:close` -  Инициируется при клике на кнопку закрытия модального окна или при клике на свободное место вокруг модального окна, снимает блокировку страницы